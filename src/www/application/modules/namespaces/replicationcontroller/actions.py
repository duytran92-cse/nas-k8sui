...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '<a class="btn btn-xs btn-primary" href="javascript:showPopup(\'/namespaces/detail/%s/replicationcontroller/view/%s\')">View</a>' % (self.params['namespace'], row['name'])
            html += '</div>'
            return html
    ...
    def create_table(self):
        ...
        table.set_title('Replication Controller')
        table.set_subtitle('List of replication controller')
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        replicationcontroller = kubeapi.list_replicationcontroller(self.params['namespace'])

        records = []
        for item in replicationcontroller['items']:
            records.append({
...
class View(common_actions.ViewAction):
    def build_view_widget(self, widget, params):
        ...
        widget.content = kubeapi.get_replicationcontroller(params['namespace'], params['name'])
