...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '<a class="btn btn-xs btn-primary" href="javascript:showPopup(\'/namespaces/detail/%s/pods/view/%s\')">View</a>' % (self.params['namespace'], row['name'])
            html += '</div>'
            return html
    ...
    def create_table(self):
        ...
        table.set_title('Pods')
        table.set_subtitle('List of pods')
        table.create_column('name', 'Name', '55%')
        table.create_column('phase', 'Phase', '10%')
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        pods = kubeapi.list_pods(self.params['namespace'])
