...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '<a class="btn btn-xs btn-primary" href="javascript:showPopup(\'/namespaces/detail/%s/statefulsets/view/%s\')">View</a>' % (self.params['namespace'], row['name'])
            html += '</div>'
            return html
    ...
    def create_table(self):
        ...
        table.set_title('Stateful Sets')
        table.set_subtitle('List of stateful set')
...
class View(common_actions.ViewAction):
    def build_view_widget(self, widget, params):
        ...
        widget.content = kubeapi.get_statefulsets(params['namespace'], params['name'])
